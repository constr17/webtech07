...
describe("JQUERY", function() {
    ...
    it("li2 check", function(done) {
        fs.readFile("./index.html", "utf-8", (err, data)=>{
            ...
            assert(li, "Exists li")
            assert(li.length > 0, "li array")
            assert(li[0].children[0].data.toLowerCase() == "odd", "item 1 text " + li[0].children[0].data)
            assert(li[1].children[0].data.toLowerCase() == "even", "item 2 text " + li[1].children[0].data)
            done()
        })
    })
})